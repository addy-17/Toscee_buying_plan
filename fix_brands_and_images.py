...
# Function to find best matching product WITHIN the same brand
...
# Process each item - use Excel brand name, find image from same brand in catalog
results = []
matched = 0
unmatched = 0
brand_mismatch = 0
...
for _, row in df.iterrows():
    ...
    # Start with Excel brand name (preserve original casing)
    ...
    # Only search within the same brand from catalog
...
print(f"Matched with images: {matched} ({new_inventory['match_rate']}%)")
print(f"Unmatched: {unmatched}")

# Show brand distribution
brands = set(item['matched_brand'] for item in results)
print(f"\nTotal brands: {len(brands)}")
print("Brands:")
for b in sorted(brands):
    count = sum(1 for item in results if item['matched_brand'] == b)
    print(f"  - {b}: {count} items")

# Show sample
print("\n=== Sample Results ===")
for r in results[:5]:
    print(f"  {r['matched_brand']} | {r['item_name'][:50]} | img={r['has_image']} | score={r['match_score']}")